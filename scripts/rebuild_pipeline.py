...
Pass `--target <T>` to scope the per-target `comparator` stage to a single mapping target.
`build` and `no_sorry` remain whole-library, and judge-package assembly stays repo-wide
(per-target judging is done downstream by `run_judge`/`score_judge --target`).
...
# Report
# ---------------------------------------------------------------------------
def render_report(stages: list[Stage], now: str, overall: str, target: str | None) -> str:
    ...
    lines += [
        ...
        "- Only the `comparator` stage is target-scoped when `--target` is given.",
...
def write_report(stages: list[Stage], now: str, overall: str, target: str | None) -> None:
    REPORT.write_text(render_report(stages, now, overall, target), encoding="utf-8")
...
def main() -> int:
    ...
    write_report(stages, now, overall, target)
